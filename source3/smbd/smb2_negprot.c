...
static NTSTATUS smb2_negotiate_context_process_posix(
    const struct smb2_negotiate_contexts *in_c, bool *posix) {
  ...
  *posix = false;
  ...
  in_posix = smb2_negotiate_context_find(in_c, SMB2_POSIX_EXTENSIONS_AVAILABLE);
  if (in_posix == NULL) {
    return NT_STATUS_OK;
  }
...
static void smbd_smb2_request_process_negprot_mc_done(
...
NTSTATUS smbd_smb2_request_process_negprot(struct smbd_smb2_request *req) {
  ...
  struct GUID in_guid;
  struct smb2_negotiate_contexts in_c = {
      .num_contexts = 0,
  };
  ...
  struct smb2_negotiate_contexts out_c = {
      .num_contexts = 0,
  };
  const struct smb311_capabilities default_smb3_capabilities =
      smb311_capabilities_parse("server", lp_server_smb3_signing_algorithms(),
  ...
  protocol = smbd_smb2_protocol_dialect_match(indyn, dialect_count, &dialect);
  ...
  if (protocol >= PROTOCOL_SMB3_11) {
    ...
    in_negotiate_context_blob =
        data_blob_const(indyn, SMBD_SMB2_IN_DYN_LEN(req));

    in_negotiate_context_blob.data += ofs;
    in_negotiate_context_blob.length -= ofs;

    status = smb2_negotiate_context_parse(req, in_negotiate_context_blob,
                                          in_negotiate_context_count, &in_c);
    if (!NT_STATUS_IS_OK(status)) {
      return smbd_smb2_request_error(req, status);
    }
  ...
  if ((dialect != SMB2_DIALECT_REVISION_2FF) &&
      (protocol >= PROTOCOL_SMB2_10) && !GUID_all_zero(&in_guid)) {
    ok = remote_arch_cache_update(&in_guid);
    if (!ok) {
      return smbd_smb2_request_error(req, NT_STATUS_UNSUCCESSFUL);
    }
  }
  ...
  {
    fstring proto;
    fstr_sprintf(proto, "SMB%X_%02X", (dialect >> 8) & 0xFF, dialect & 0xFF);
  ...
  in_preauth =
      smb2_negotiate_context_find(&in_c, SMB2_PREAUTH_INTEGRITY_CAPABILITIES);
  if (protocol >= PROTOCOL_SMB3_11 && in_preauth == NULL) {
    return smbd_smb2_request_error(req, NT_STATUS_INVALID_PARAMETER);
  }
  in_cipher = smb2_negotiate_context_find(&in_c, SMB2_ENCRYPTION_CAPABILITIES);
  in_sign_algo = smb2_negotiate_context_find(&in_c, SMB2_SIGNING_CAPABILITIES);
  ...
  if (in_preauth != NULL) {
    ...
    if (selected_preauth == 0) {
      return smbd_smb2_request_error(
          req, NT_STATUS_SMB_NO_PREAUTH_INTEGRITY_HASH_OVERLAP);
    }
    ...
    status = smb2_negotiate_context_add(
        req, &out_c, SMB2_PREAUTH_INTEGRITY_CAPABILITIES, buf, sizeof(buf));
    if (!NT_STATUS_IS_OK(status)) {
      return smbd_smb2_request_error(req, status);
    }
  ...
  if ((capabilities & SMB2_CAP_ENCRYPTION) && (in_cipher != NULL)) {
    ...
    if (srv_preferred_idx != UINT16_MAX) {
      xconn->smb2.server.cipher = srv_ciphers->algos[srv_preferred_idx];
    }
    ...
    status = smb2_negotiate_context_add(
        req, &out_c, SMB2_ENCRYPTION_CAPABILITIES, buf, sizeof(buf));
    if (!NT_STATUS_IS_OK(status)) {
      return smbd_smb2_request_error(req, status);
    }
  }
  ...
  if (in_sign_algo != NULL) {
    ...
    if (srv_preferred_idx != UINT16_MAX) {
      ...
      xconn->smb2.server.sign_algo = srv_sign_algos->algos[srv_preferred_idx];

      SSVAL(buf, 0, 1); /* SigningAlgorithmCount */
      SSVAL(buf, 2, xconn->smb2.server.sign_algo);

      status = smb2_negotiate_context_add(
          req, &out_c, SMB2_SIGNING_CAPABILITIES, buf, sizeof(buf));
      if (!NT_STATUS_IS_OK(status)) {
        return smbd_smb2_request_error(req, status);
      }
    }
  ...
  status = smb311_capabilities_check(
      &default_smb3_capabilities, "smb2srv_negprot", DBGLVL_NOTICE,
      NT_STATUS_INVALID_PARAMETER, "server", protocol,
      xconn->smb2.server.sign_algo, xconn->smb2.server.cipher);
  if (!NT_STATUS_IS_OK(status)) {
    return smbd_smb2_request_error(req, status);
  }
  ...
  if (posix) {
    ...
    status = smb2_negotiate_context_add(
        req, &out_c, SMB2_POSIX_EXTENSIONS_AVAILABLE, buf, 16);
    if (!NT_STATUS_IS_OK(status)) {
      return smbd_smb2_request_error(req, status);
    }
  ...
  if (out_c.num_contexts != 0) {
    status =
        smb2_negotiate_context_push(req, &out_negotiate_context_blob, out_c);
    if (!NT_STATUS_IS_OK(status)) {
      return smbd_smb2_request_error(req, status);
    }
  }
  ...
  if (out_negotiate_context_blob.length != 0) {
    ...
    ok = data_blob_append(req, &outdyn, out_negotiate_context_blob.data,
  ...
  memcpy(outbody.data + 0x08, out_guid_blob.data, 16); /* server guid */
  SIVAL(outbody.data, 0x18, capabilities);             /* capabilities */
  ...
  SSVAL(outbody.data, 0x38, security_offset); /* security buffer offset */
  ...
  xconn->smb2.client.dialects = talloc_array(xconn, uint16_t, dialect_count);
  ...
  if (!xconn->client->server_multi_channel_enabled) {
     ...
     * the correct value.
     */
    xconn->client->global->client_guid = xconn->smb2.client.guid;
    return smbd_smb2_request_done(req, outbody, &outdyn);
  }
  ...
  *state = (struct smbd_smb2_request_process_negprot_state){
  ...
  subreq = smb2srv_client_mc_negprot_send(state, req->xconn->client->raw_ev_ctx,
                                          req);
  if (subreq == NULL) {
    return smbd_smb2_request_error(req, NT_STATUS_NO_MEMORY);
  }
  tevent_req_set_callback(subreq, smbd_smb2_request_process_negprot_mc_done,
                          state);
  return NT_STATUS_OK;
}
...
static void smbd_smb2_request_process_negprot_mc_done(
  ...
  if (NT_STATUS_EQUAL(status, NT_STATUS_MESSAGE_RETRIEVED)) {
     ...
     * is more useful.
     */
    smbXsrv_connection_disconnect_transport(xconn, NT_STATUS_CONNECTION_IN_USE);
    smbd_server_connection_terminate(xconn, "passed connection");
    /*
...
DATA_BLOB negprot_spnego(TALLOC_CTX *ctx, struct smbXsrv_connection *xconn) {
  ...
  status = auth_generic_prepare(talloc_tos(), xconn->remote_address,
                                xconn->local_address, "SMB", &gensec_security);
  ...
  if (NT_STATUS_IS_OK(status)) {
    ...
    if (NT_STATUS_IS_OK(status)) {
      status = gensec_update(gensec_security, ctx, data_blob_null, &blob);
  ...
  /* strangely enough, NT does not sent the single OID NTLMSSP when
     ...
     OLD COMMENT : "we can't do this until we teach our session setup
     parser to know about raw NTLMSSP (clients send no ASN.1 wrapping if
     we do this)"

     Our sessionsetup code now handles raw NTLMSSP connects, so we can go
     back to doing what W2K3 does here. This is needed to make PocketPC
     2003 CIFS connections work with SPNEGO. See bugzilla bugs #1828 and
     #3133 for details. JRA.
...
NTSTATUS smb2_multi_protocol_reply_negprot(struct smb_request *req) {
  ...
  while (smbreq_bufrem(req, p) > 0) {
    ...
    tmp = talloc_realloc(talloc_tos(), cliprotos, char *, num_cliprotos + 1);
    if (tmp == NULL) {
