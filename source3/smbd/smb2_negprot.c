...
static NTSTATUS smb2_negotiate_context_process_posix(
    ...
    *posix = false;
    ...
    in_posix =
        smb2_negotiate_context_find(in_c, SMB2_POSIX_EXTENSIONS_AVAILABLE);
    if (in_posix == NULL) {
        return NT_STATUS_OK;
    }
...
NTSTATUS smbd_smb2_request_process_negprot(struct smbd_smb2_request *req) {
    ...
    if (protocol >= PROTOCOL_SMB3_11) {
        ...
        status = smb2_negotiate_context_parse(
            req, in_negotiate_context_blob, in_negotiate_context_count, &in_c);
        if (!NT_STATUS_IS_OK(status)) {
            return smbd_smb2_request_error(req, status);
        }
    ...
    {
        fstring proto;
        fstr_sprintf(proto, "SMB%X_%02X", (dialect >> 8) & 0xFF,
                     dialect & 0xFF);
    ...
    in_cipher =
        smb2_negotiate_context_find(&in_c, SMB2_ENCRYPTION_CAPABILITIES);
    in_sign_algo =
        smb2_negotiate_context_find(&in_c, SMB2_SIGNING_CAPABILITIES);
    ...
    if (out_c.num_contexts != 0) {
        status = smb2_negotiate_context_push(req, &out_negotiate_context_blob,
                                             out_c);
        if (!NT_STATUS_IS_OK(status)) {
            return smbd_smb2_request_error(req, status);
        }
    ...
    *state = (struct smbd_smb2_request_process_negprot_state){
    ...
    subreq = smb2srv_client_mc_negprot_send(
        state, req->xconn->client->raw_ev_ctx, req);
    if (subreq == NULL) {
        return smbd_smb2_request_error(req, NT_STATUS_NO_MEMORY);
    }
...
static void smbd_smb2_request_process_negprot_mc_done(
    ...
    struct smbd_smb2_request_process_negprot_state *state =
        tevent_req_callback_data(
            subreq, struct smbd_smb2_request_process_negprot_state);
...
DATA_BLOB negprot_spnego(TALLOC_CTX *ctx, struct smbXsrv_connection *xconn) {
    ...
    status =
        auth_generic_prepare(talloc_tos(), xconn->remote_address,
...
NTSTATUS smb2_multi_protocol_reply_negprot(struct smb_request *req) {
    ...
    while (smbreq_bufrem(req, p) > 0) {
        ...
        tmp =
            talloc_realloc(talloc_tos(), cliprotos, char *, num_cliprotos + 1);
        if (tmp == NULL) {
