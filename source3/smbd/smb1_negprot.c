...
static void get_challenge(struct smbXsrv_connection *xconn, uint8_t buff[8]) {
    ...
    nt_status = make_auth4_context(xconn, &xconn->smb1.negprot.auth_context);
...
static NTSTATUS reply_nt1(struct smb_request *req, uint16_t choice) {
    ...
    if (xconn->smb1.negprot.encrypted_passwords &&
        ...
        SSVAL(req->outbuf, smb_flg2, req->flags2 | FLAGS2_EXTENDED_SECURITY);
    }
    ...
    if (lp_nt_status_support()) capabilities |= CAP_STATUS32;

    if (lp_host_msdfs()) capabilities |= CAP_DFS;
    ...
    put_long_date_full_timespec(TIMESTAMP_SET_NT_OR_BETTER,
                                (char *)req->outbuf + smb_vwv11 + 1, &ts);
    ...
    if (!negotiate_spnego) {
        ...
        if (xconn->smb1.negprot.encrypted_passwords) {
            ...
            ret = message_push_blob(&req->outbuf,
                                    data_blob_const(chal, sizeof(chal)));
            if (ret == -1) {
